...
def start_chat(payload: QueryRequest) -> QueryResponse:
    ...
    try:
        chat_id, answer = rag_services.start_new_chat(payload.query)
    except FileNotFoundError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    except Exception as exc:  # pragma: no cover - defensive logging
        logger.exception("Graph RAG start_chat failure")
        raise HTTPException(status_code=500, detail="Failed to process query") from exc

    return QueryResponse(chat_id=chat_id, response=answer)
...
def continue_existing_chat(chat_id: UUID, payload: QueryRequest) -> QueryResponse:
    ...
    try:
        answer = rag_services.continue_chat(chat_id=chat_id, user_query=payload.query)
    except FileNotFoundError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    except Exception as exc:  # pragma: no cover - defensive logging
        logger.exception("Graph RAG continue_chat failure")
        raise HTTPException(status_code=500, detail="Failed to process query") from exc
...
def embed_corpus(
    ...
    try:
        embedded_count = rag_services.embed_documents(
            ...
            overlap=payload.overlap or 128,
            batch_size=payload.batch_size or 256,
        )
    except FileNotFoundError as exc:
